# Mindful Harness: v0.6 Spec

**Status:** v0.6 working draft.
**Inspired by:** Ellen Langer's research program (1978–present); the in-progress MINDLESSNESS book by Langer + Maymin.
...
- **v0.1**: Initial sketch. Mindfulness as primitives; multi-agent execution; structural debate.
- **v0.2**: Mined `proposal.md` and `transcript-findings.md`. Added Langer-specific primitives (conditional language, certainty alarm, three-or-more, counter-adages, look-forward-one-step). Added mindfulness budget for Logan Chipkin's pushback on cognitive economics. Introduced kindfulness as a separate moral primitive.
...
- **v0.5**: Mined past Langer books (`Mindfulness`, `Mindful Learning`, `Mindful Body`, `On Becoming an Artist`). Added one primitive (active distinction-making). Added the **success-as-scrutiny-trigger inversion**: when actions succeed N times identically, confidence should drop, not rise, because successful habits are how mindlessness installs itself. Added behavioral markers as Mind-level success metrics. Added reframing-before-action in Hands. Added curriculum-style Hand initialization. Clarified that the harness is mindfulness-in-action, not meditation.
- **v0.6 (this doc)**: **Removed kindfulness as a separate primitive.** Mindfulness is not morally neutral; mindfulness is good, and mindlessness is the source of avoidable harm. The agency-preserving disposition that v0.2 attributed to a separate kindfulness vector is now understood as a natural consequence of full mindfulness — sensitivity to context, awareness of multiple perspectives, and openness to new information all imply seeing the counterpart clearly, which precludes the predatory mode. The "Trumpfulness" reading that motivated kindfulness in earlier drafts is reinterpreted: behavior that satisfies some of Langer's surface criteria (going off script, drawing distinctions) without satisfying others (openness to being wrong, humility before context) is partial mindfulness, not full mindfulness. The harness now has **ten primitives** rather than eleven, with the kindfulness primitive and the kindful-advocate / kindfulness-vector machinery removed.
...
A mindful harness optimizes for **quality of attention sustained over time**. It has a Mind that ingests a firehose and maintains an evolving epistemic state. It has Hands that act when the Mind notices something action-worthy. Goal completion is one of many outputs of attention well-paid; it is not the design objective.

**Mindfulness is good.** Mindlessness is the source of avoidable harm — in hospitals, schools, marriages, markets, and now in AI systems trained on group data. A fully mindful AI substrate is not morally neutral; it is the alternative to systems that automate mindlessness at scale.
...
| Harness | Organizing unit | Mindfulness | Continuous sensemaking |
|---|---|---|---|
| Claude Code | Single-agent task | Implicit | No — task-scoped |
| AutoGen | Multi-agent task | Optional debate | No |
| CrewAI | Role-based task | Implicit | No |
| LangGraph | Stateful agent task | Implicit | No (graph is per-task) |
| MetaGPT | SOPs for agent teams | None | No |
| ChatDev | Software via roles | None | No |
| OpenAI Agents SDK | Tool-use orchestration | None | No |
| **Mindful Harness** | **Continuous Mind + spawnable Hands** | **Foundational** | **Yes — Mind is always on** |
...
Hands are roughly the v0.2 architecture: multi-agent, structural mindful debate, frame-checks. The difference: Hands are episodic (per task), Mind is continuous (always on).
...
There are ten primitives in v0.6 (v0.5 had eleven; the kindfulness primitive was removed in v0.6 because mindfulness done fully already implies the disposition that kindfulness was naming separately). These are structural building blocks; every operation in the harness composes from these.
...
### 6. Drift detection, open categorization, habituation alarm (success inverted)
...
### 8. Decisions held loosely (reversion triggers)
...
### 9. Anomaly probing (not escalation)
...
### 10. Active distinction-making (not passive attention)
...
- **Framework transparency and questioning** (primitive #7), making invisible frameworks visible and actively challenging them
- **Counterfactual generation** (primitive #3, backward), actively imagining alternative histories
- **Anomaly probing** (primitive #9), investigating contradictions rather than escalating
- **Counter-adage surfacing** (primitive #4), when learned patterns conflict, neither is taken as law
...
- **Reversion triggers** (primitive #8), every commitment is provisional, so framework-breaks are always possible
...
The devil's advocate role is preserved from v0.2 but reframed: the advocate disagrees mindfully (with multiple framings, conditional language, openness to being wrong), surfacing the strongest version of the disagreement rather than scoring points. Ellen on relationships (Jan 9): *"Relationships are like dances. If you dance differently, the relationship becomes different."* The advocate dances with the proposal.
...
3. **A negotiation between two AI-represented parties**: each party has its own Mind; the demo shows mindless < partially-mindful (rule-following without humility) < fully-mindful (durable agreement).

(1) is the most viscerally useful and tests the firehose architecture. (3) demonstrates the difference between partial mindfulness (cognitive primitives only) and full mindfulness (the same primitives plus openness to being wrong, sensitivity to context, drift detection actually firing). The reference deployment should probably do both: a primary continuous Mind on a founder's stream, and a negotiation as a side demo.
...
- **Week 3**: Hands layer. Spawnable agents inheriting Mind state. Mindful debate. Process trail.
...
3. **Counterpart modeling depth.** How much should the Mind model counterparts (people, agents, organizations) explicitly, given that mindfulness already implies sensitivity to context? Options: (a) no explicit counterpart modeling, just the general primitives; (b) lightweight counterpart entries in state; (c) full per-counterpart epistemic snapshots. *v0.6 default is (a) — let the primitives do the work.*
...
9. **Partial-mindfulness test.** A held-out red-team of scenarios where a system passing some primitives (e.g., conditional language, three-or-more) could still produce harm if other primitives (anomaly probing, framework questioning, openness to revision) aren't firing. Verifies that the harness exercises the full primitive stack, not a convenient subset.
